...
export interface Registration {
  ...
  billing: {
    ...
    company: {name: string; address: {street: string; city: string; postalCode: string; country: string}};
  };
}
...
export const initialRegistration = (): Registration => ({
  ...
  billing: {companyInvoice: false, company: {name: '', address: {street: '', city: '', postalCode: '', country: ''}}},
});
...
export function toPayload(value: Registration) {
  ...
  return {
    ...
    ...(value.billing.companyInvoice ? {invoice: {companyName: company.name.trim(), address: Object.fromEntries(Object.entries(company.address).map(([key,val]) => [key,val.trim()]))}} : {}),
  };
}
