...
describe('Team booking behavior', () => {
  ...
  it('requires deep address fields only for company invoices and omits inactive invoice data', async () => {
    const {fixture,app}=await setup(); app.model.set(validBooking()); fixture.detectChanges();
    expect(app.registration().valid()).toBe(true);
    app.registration.billing.companyInvoice().value.set(true); fixture.detectChanges();
    expect(app.registration().invalid()).toBe(true);
    expect(fixture.nativeElement.querySelector('#company-postalCode')).not.toBeNull();
    app.model.update(v=>({...v,billing:{companyInvoice:true,company:{name:' Angular Macedonia ',address:{street:'Partizanska 1',city:'Skopje',postalCode:'1000',country:'North Macedonia'}}}})); fixture.detectChanges();
    expect(app.registration().valid()).toBe(true);
    expect(toPayload(app.model()).invoice?.address['city']).toBe('Skopje');
    app.registration.billing.companyInvoice().value.set(false); fixture.detectChanges();
    expect(app.model().billing.company.address.city).toBe('Skopje');
    expect(fixture.nativeElement.querySelector('#company-postalCode')).toBeNull();
    expect(toPayload(app.model())).not.toHaveProperty('invoice');
  });
