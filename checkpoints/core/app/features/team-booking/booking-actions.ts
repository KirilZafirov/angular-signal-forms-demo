...
type Outcome = {kind: 'idle'} | {kind: 'failed'; message: string} | {kind: 'rejected'; attendeeId: string} | {kind: 'saved'; reference: string; snapshot: string};
...
export class BookingActions {
  ...
  readonly message = computed(() => {
    ...
    if (result.kind === 'saved') return JSON.stringify(toPayload(this.model())) === result.snapshot
    ...
    if (result.kind === 'rejected') {
      ...
      return this.registration.attendees[index].email().errors().some(e => e.kind === 'duplicate')
  ...
  addAttendee() {
    ...
    afterNextRender(() => this.registration.attendees[this.model().attendees.length - 1].name().focusBoundControl(), {injector: this.injector});
  }
  removeAttendee(id: string) {
    ...
    afterNextRender(() => this.registration.attendees[Math.min(index, this.model().attendees.length - 1)].name().focusBoundControl(), {injector: this.injector});
  }
      ...
      action: async (field) => {
        ...
        try {
          ...
          if (result.kind === 'rejected') {
            ...
            return {fieldTree: field.attendees[index].email, kind: 'duplicate', message: result.message};
          }
          this.outcome.set({kind: 'saved', reference: result.reference, snapshot: JSON.stringify(payload)});
          return undefined;
        } catch {
          this.outcome.set({kind: 'failed', message: 'Could not confirm the reservation. Your entries are preserved. Retry when ready.'});
